...
RectangleRenderComponent::RectangleRenderComponent() :m_pTransform(nullptr), m_pRectangle(nullptr) {}

void RectangleRenderComponent::OnEvent(const std::string& event, const YY::VarList& args)
{
	if (event == "addComponent")
	{
		m_pTransform = (ItemTransformComponent*)GetOwner()->FindComponent("ItemTransformComponent");
		m_pRectangle = (RectangleComponent*)GetOwner()->FindComponent("RectangleComponent");
...
void RectangleRenderComponent::OnAddBatch(IBatchGroup* pBatchGroup)
{
	pBatchGroup->AddGUIBatch(this);
}

void RectangleRenderComponent::Setup()
{
	if (nullptr == m_pTransform || nullptr == m_pRectangle)
		return;
	...
	m_vertices[0].color = m_pRectangle->GetColor();
	m_vertices[1].color = m_pRectangle->GetColor();
	m_vertices[2].color = m_pRectangle->GetColor();
	m_vertices[3].color = m_pRectangle->GetColor();
...
void RectangleRenderComponent::OnRender(IRender* pRender, RenderContext* pCxt)
{
	if (nullptr == m_pTransform || nullptr == m_pRectangle)
		return;
